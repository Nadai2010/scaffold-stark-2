...
import { KatanaAccount, katanaAccounts } from "~~/utils/katanaAccounts";
import {
  RequestFnCall,
  RpcMessage,
  RpcTypeToMessageMap,
...
export const burnerKatanaWalletId = "katana-wallet";
export const burnerKatanaWalletName = "Katana Wallet";
const burnerKatanaWalletIcon =
...
export class KatanaConnector extends InjectedConnector {
  chain: Chain = chains.katanaFork;
  katanaAccount: KatanaAccount = katanaAccounts[0];

  constructor() {
    super({
      options: {
        id: burnerKatanaWalletId,
        name: burnerKatanaWalletName,
        icon: { dark: burnerKatanaWalletIcon, light: burnerKatanaWalletIcon },
      },
    });
  ...
  async account(): Promise<AccountInterface> {
    return Promise.resolve(
      new Account(
        ...
        this.katanaAccount.accountAddress,
        this.katanaAccount.privateKey,
        "1",
      ),
    );
  }
  ...
  get icon(): ConnectorIcons {
    return {
      dark: burnerKatanaWalletIcon,
      light: burnerKatanaWalletIcon,
    };
  }
